...
function SoloApp({ theme, onToggleTheme, onExit }) {
  ...
  const completeSession = ({ book, mission, memo, action, minutes }) => {
    const { breakdown, xpGained } = DQ.calcSessionXp({ mission, memo, action, minutes });
      ...
      <div className="mx-auto max-w-2xl space-y-5 px-4 py-6 sm:py-8">
        ...
        {screen.name === "home" && (
          <React.Fragment>
            ...
            />
            <DQ.CheerLetters letters={DQ.loadLetters()} />
...
function App() {
  ...
  const [start, setStart] = React.useState(circleStart);
  const [mode, setMode] = React.useState(start ? "circle" : null);

  // 自分から退出したら、自動復帰の情報も捨てる
  // (捨てないと、もう一度「読書会」を選んだときに同じ部屋へ戻ってしまう)
  const leaveCircle = () => {
    setStart(null);
    setMode(null);
  };
