...
// ---------- 応援レター(読書会でもらった応援コメントの保管箱) ----------
DQ.CheerLetters = function CheerLetters({ letters }) {
  const [showAll, setShowAll] = React.useState(false);
  if (letters.length === 0) return null;
  const shown = showAll ? letters : letters.slice(0, 5);

  return (
    <section className="animate-fade-up rounded-2xl border-2 border-pink-300 bg-gradient-to-br from-pink-50 to-amber-50 p-4 shadow-md sm:p-5 dark:border-pink-800 dark:from-stone-800 dark:to-pink-950/40">
      <h2 className="font-pixel text-lg text-stone-800 dark:text-pink-100">
        💌 応援レター({letters.length}通)
      </h2>
      <p className="mt-1 text-xs text-stone-500 dark:text-stone-400">
        読書会で仲間から届いた、あなたへの応援です
      </p>
      <ul className="mt-3 space-y-2">
        {shown.map((l) => (
          <li
            key={l.key}
            className="rounded-xl border border-pink-200 bg-white/80 p-3 dark:border-pink-900 dark:bg-stone-800/70"
          >
            <p className="text-sm font-bold text-stone-800 dark:text-stone-100">
              「{l.comment}」
            </p>
            <p className="mt-1 text-xs text-stone-500 dark:text-stone-400">
              {l.gifterName} さんより ・ 『{l.bookTitle}』 ・{" "}
              {new Date(l.at).toLocaleDateString("ja-JP")}
            </p>
          </li>
        ))}
      </ul>
      {letters.length > 5 && (
        <button
          ...
          className="mt-2 text-sm text-pink-700 underline dark:text-pink-300"
        >
          {showAll ? "とじる" : `すべて見る(${letters.length}通)`}
        </button>
      )}
    </section>
  );
...
// backLabel / startLabel で、戻る・決定ボタンの文言を場面に合わせて変えられる
DQ.MissionDraw = function MissionDraw({
  book,
  onStart,
  onBack,
  hideTime,
      ...
      {mission === null ? (
        ...
        <div className="space-y-4">
          ...
          <div className="flex justify-center gap-3">
            ...
            >
              {startLabel || (hideTime ? "このカードで読む" : "クエスト開始!")}
            </button>
          </div>
        </div>
      )}
...
// ---------- 読書メモ入力 ----------
DQ.MemoForm = function MemoForm({ book, mission, minutes, onComplete, submitLabel }) {
  const [memo, setMemo] = React.useState("");
  const [action, setAction] = React.useState("");
  const memoRef = React.useRef(null);

  const hints = mission.hints || DQ.DEFAULT_HINTS;
  const len = memo.trim().length;
  const { xpGained } = DQ.calcSessionXp({ mission, memo, action, minutes });
  // 次の「じっくり書いたボーナス」まであと何字か
  const nextTier = [...DQ.DEEP_TIERS].reverse().find((t) => len < t.chars);

  // 書き出しヒントをメモの末尾に差し込む(改行でつなぐ)
  ...
  // 差し込んだ文字が画面に反映された直後に、カーソルを末尾へ置いてそのまま続きを書けるようにする
    ...
    <section className="animate-fade-up space-y-4">
      ...
      <div className="rounded-2xl border-2 border-amber-300 bg-white/80 p-4 shadow-md dark:border-amber-700 dark:bg-stone-800/80">
        ...
        <label className="mt-4 block text-sm font-bold text-stone-600 dark:text-stone-300">
          🚶 明日の一歩(任意・+{DQ.ACTION_BONUS_XP} EXP)
        </label>
        <input
          type="text"
          value={action}
        ...
        <p className="mt-4 text-center text-sm text-stone-600 dark:text-stone-300">
          このまま完了すると <span className="font-bold text-orange-600 dark:text-orange-400">+{xpGained} EXP</span>
        </p>
        <div className="mt-2 flex flex-col gap-2 sm:flex-row-reverse">
          <button
            type="button"
            onClick={() => onComplete(memo, action)}
            disabled={!memo.trim()}
